...
void Viewer::Run()
{
    ...
    pangolin::CreateWindowAndBind("Dynamic-SLAM: Map Viewer",1024,768);
    ...
    pangolin::Var<bool> menuShowLabels("menu.Show Labels",true,true);
    ...
    cv::namedWindow("Dynamic-SLAM: Current Frame");
    ...
    while(1)
    {
        ...
        if(menuStop){
            //cout << "mpSystem->SetStop(true)" << endl;
            mpSystem->SetStop(true);
            usleep(300);
        }
        else{
...
/*
void Viewer::SetStop(bool& isStop){
    if(mbmenuStop)
        isStop = true;
    else
        isStop = false;
