...

{
    ...
    /// new
    mvDynamic=frame.mvDynamic;
    msDynamicLabel=frame.msDynamicLabel;
    mmDynamicBoundary=frame.mmDynamicBoundary;
    mmDynamicKeys=frame.mmDynamicKeys;
    mmDynamicDepth=frame.mmDynamicDepth;
}
    ...
    mpReferenceKF(static_cast<KeyFrame*>(NULL))
{
    ...
    //cout << "id:" << mnId << endl;
    ComputeStereoMatches(imSemantic);
    //cout << "=======" << endl;
...
void Frame::ComputeStereoMatches(const cv::Mat &imSemantic)
{
    mvuRight = vector<float>(N,-1.0f);
    mvDepth = vector<float>(N,-1.0f);
    mvDynamic = vector<bool>(N,0);
    ...
    for(int iL=0; iL<N; iL++)
    {
        ...
        /// new
        int label=imSemantic.at<char16_t>(vL,uL);
        if(label>24000 && label<27000){
            mvDynamic[iL] = true;
            //cout << vL << ", " << uL << "  |  ";
            continue;
        }
